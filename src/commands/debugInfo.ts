import { version, START_TIME, FAVORITE_NUMBER, getFormattedStartTime } from '../config';
...
export async function getDebugInfo(): Promise<string> {
	...
	return [
		...
		...([
			...
			['Running Since', getFormattedStartTime()],
			['Hostname', hostname()],
			['Ultimate Answer', '42'],
			'',
			['Current Zoom Status', active ? `Active, ${participants} participants` : `Inactive`],
			'',
			process.env.HEROKU_SLUG_COMMIT && ['Git Commit ID', process.env.HEROKU_DYNO_ID],
			'',
