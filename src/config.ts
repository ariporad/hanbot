...
export const ZOOM_MEETING_ID = getEnvVar(
  "ZOOM_MEETING_ID",
  "Need a Zoom Meeting ID to Monitor!"
);

const ZOOM_API_KEY = getEnvVar("ZOOM_API_KEY", "Need a Zoom JWT API Key!");

const ZOOM_API_SECRET = getEnvVar(
  "ZOOM_API_SECRET",
  "Need a Zoom API Secret Token!"
);
...
export const ZOOM_TIME_THRESHOLD =
  parseInt(process.env.ZOOM_TIME_THRESHOLD || "NaN", 10) || null;
...
export const DISCORD_TOKEN = getEnvVar(
  "DISCORD_TOKEN",
  "Need a Discord Bot User Secret!"
);
...
if (!DISCORD_ADMITTED_ROLE) {
  console.warn(
    "WARNING: $DISCORD_ADMITTED_ROLE is not set. No role will be added to new users."
  );
}
